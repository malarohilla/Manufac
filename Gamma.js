...
class GammaStatistics extends React.Component {
    calculateGamma(dataset) {
        ...
        const dataWithGamma = dataset.map((data) => {
        ...
        const classGamma = {};
        dataWithGamma.forEach((data) => {
            const { Alcohol, Gamma } = data;
        ...
        const result = Object.keys(classGamma).map((alcoholClass) => {
            const gammas = classGamma[alcoholClass];
            ...
            const median =
                sortedGammas.length % 2 === 0
                    ? (sortedGammas[sortedGammas.length / 2 - 1] +
                        sortedGammas[sortedGammas.length / 2]) /
                    2
                    : sortedGammas[Math.floor(sortedGammas.length / 2)];


            const mode = sortedGammas.reduce(
                (accumulator, gamma) => {
                    accumulator.count[gamma] = (accumulator.count[gamma] || 0) + 1;
                    if (accumulator.count[gamma] > accumulator.maxCount) {
                        accumulator.maxCount = accumulator.count[gamma];
                        accumulator.modes = [this.fixedPointVal(gamma)];
                    } else if (accumulator.count[gamma] === accumulator.maxCount) {
                        accumulator.modes.push(this.fixedPointVal(gamma));
                    }
                    return accumulator;
                },
                { count: {}, maxCount: 0, modes: [] }
            ).modes;
    ...
    render() {
        ...
        const gammaStats = this.calculateGamma(data);
        console.log(gammaStats)
        return (
            <div className='gamma'>
                <h1>Gamma</h1>
                <table>
                    ...
                    <tbody>
                        {gammaStats.map((stats) => (
                            <tr >
                                <td>{`class ${stats.Alcohol}`}</td>
                                <td>{this.fixedPointVal(stats.Mean)}</td>
                                <td>{this.fixedPointVal(stats.Median)}</td>
                                <td>{stats.Mode}</td>
